...
public class InvoiceController {
    ...
    @Operation(
        ...
        description = "Generates an invoice for a given account and payment method",
        requestBody = @RequestBody(
            description = "Account and payment method details",
            required = true,
    ...
    public ResponseEntity<InvoiceDTO> createInvoice(@RequestBody InvoiceRequest request) {
        try {
            logger.info("Received request to create invoice for account ID: {}", request.getAccount().getId());
            InvoiceDTO invoice = invoiceService.createInvoice(request.getAccount(), request.getPaymentMethod());
