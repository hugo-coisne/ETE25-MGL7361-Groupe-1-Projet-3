...
public class Main {
    ...
    // public static void scenario() throws Exception { // scenario described in the
    ...
    // String password = "P@ssword123";
    //
    // Main.scenarioStep("1. L'utilisateur devrait créer un compte et se connecter
    ...
    // Main.scenarioDetailsStep(publisher.getName()));
    //
    // // Livres
    ...
    //
    // Main.scenarioStep("2.2.2 Rechercher les livres à partir d'un titre ('Les
    // Misérables')");
    // booksByTitle = bookAPI.getBooksBy(Map.of(
    // BookProperty.TITLE, "Les Misérables"));
    // booksByTitle.forEach(book -> Main.scenarioDetailsStep(book.getTitle()));
    //
    // Main.scenarioStep("2.2.3 Rechercher les livres à partir d'un titre partiel
    // ('The')");
    // booksByTitle = bookAPI.getBooksBy(Map.of(
    // BookProperty.TITLE, "The"));
    // booksByTitle.forEach(book -> Main.scenarioDetailsStep(book.getTitle()));
    //
    // Main.scenarioStep("2.2.4 Rechercher les livres à partir d'un auteur
    // ('Camus')");
    // booksByTitle = bookAPI.getBooksBy(Map.of(
    // BookProperty.AUTHOR, "Camus"));
    // booksByTitle.forEach(book -> Main.scenarioDetailsStep(book.getTitle()));
    //
    // Main.scenarioStep("2.2.5 Rechercher les livres à partir d'un editeur
    // ('Gal')");
    // booksByTitle = bookAPI.getBooksBy(Map.of(
    ...
    // );
    //
    // Main.scenarioStep("2.2.6 Rechercher les livres à partir d'un mélange de
    // caractéristiques" +
    // " - titre(partiel) : 'Lover', " +
    // "ISBN(partiel): '06951', " +
    // "Editeur: 'Seuil', " +
    ...
    //
    // // Livre pas en stock
    // Main.scenarioDetailsStep("Le livre The Lover n'est pas en stock donc l'ajout
    // au panier n'est pas possible");
    // BookDTO book = booksByTitle.getFirst();
    // cartAPI.add(book, account);
    // Main.scenarioDetailsStep("Contenu du panier : " + cartAPI.getCart(account));
    //
    // // Livre devrait être en stock (first run ie il n'y a pas trop de livre dans
    // le panier et ça ne dépasse pas le stock)
    // Main.scenarioDetailsStep("Le livre Les Misérables est en stock et dois bien
    // s'ajouter au panier (s'il n'y en a pas trop dans le panier)");
    // book = bookAPI.getBooksBy(Map.of(BookProperty.TITLE, "Les
    // Misérables")).getFirst();
    // cartAPI.add(book, account);
    // Main.scenarioDetailsStep("Contenu du panier : " + cartAPI.getCart(account));
    //
    ...
    // Main.scenarioDetailsStep("Contenu du panier : " + cartAPI.getCart(account));
    //
    // Main.scenarioStep("4. Supprimer, du panier, un ou des livres qu'on ne désire
    // plus acheter;");
    // Main.scenarioStep("4.1 Suppression du dernier livre ajouté ('The
    // Stranger')");
    ...
    // BookDTO lesMiserables = bookAPI.getBooksBy(Map.of(BookProperty.TITLE, "Les
    // Misérables")).getFirst();
    ...
    // account = accountClient.signin(email, password);
    // Main.scenarioDetailsStep("Connecté avec le compte - " + account);
    //
    // Main.scenarioStep("5.2 Recherche de livre (titre: 'The')");
    // booksByTitle = bookAPI.getBooksBy(Map.of(
    // BookProperty.TITLE, "The"));
    // booksByTitle.forEach(bookFor ->
    // Main.scenarioDetailsStep(bookFor.getTitle()));
    //
    ...
    // Main.scenarioDetailsStep("Contenu du panier : " + cartAPI.getCart(account));
    //
    // Main.scenarioStep("5.4 Supprimer, du panier, un ou des livres qu'on ne désire
    // plus acheter;");
    // Main.scenarioDetailsStep("Suppression du dernier livre ajouté ('The
    // Stranger')");
    // cartAPI.remove(book, account);
    // Main.scenarioDetailsStep("Contenu du panier : " + cartAPI.getCart(account));
    //
    ...
    // Main.scenarioDetailsStep("Contenu du panier : " + cartAPI.getCart(account));
    //
    // Main.scenarioStep("À partir du panier, passer au paiement et régler la
    // facture; les livres du panier\n" +
    // "constituent alors une nouvelle commande où chaque livre de la commande a un
    // status \"En\n" +
    // "attente de livraison\"");
    //
    // InvoiceAPIImpl invoiceAPI = new InvoiceAPIImpl();
    // Main.scenarioStep("6.1 Simulation de paiement réussi (par carte), génération
    // de la facture et de la commande");
    ...
    // pendingDelivery.getOrder().getOrderNumber() +
    // ", Status: " + pendingDelivery.getDeliveryStatus());
    // }
    //
