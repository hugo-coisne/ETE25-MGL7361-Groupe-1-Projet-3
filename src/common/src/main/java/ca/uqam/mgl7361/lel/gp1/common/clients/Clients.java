...
public class Clients {
        ...
        static String checkoutServiceUrl = dotenv.get("CHECKOUT_SERVICE_URL", "http://localhost:8085");
        ...
        public static final CheckoutAPIClient checkoutClient = Feign.builder()
                        .encoder(new JacksonEncoder())
                        .decoder(new JacksonDecoder())
                        .target(CheckoutAPIClient.class, checkoutServiceUrl);

        public static final InvoiceAPIClient invoiceClient = Feign.builder()
                        .encoder(new JacksonEncoder())
                        .decoder(new JacksonDecoder())
                        .target(InvoiceAPIClient.class, checkoutServiceUrl);
}
